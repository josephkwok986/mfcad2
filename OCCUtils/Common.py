...
def adapt_edge_to_hcurve(edg):
    """
    Historical helper that used to return a BRepAdaptor_HCurve handle.
    pythonocc-core 7.7 removed that wrapper, so we now directly return
    a BRepAdaptor_Curve initialised with the given edge.
    """
    return BRepAdaptor_Curve(edg)
